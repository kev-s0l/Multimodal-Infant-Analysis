...
parser = argparse.ArgumentParser(description="CoV + Sleep Stage Synchronization Analysis")
parser.add_argument("--h5", required=True, help="Path to H5 movement file")
parser.add_argument("--sensor", required=True, help="Sensor ID (e.g., 16162 or XI-016162)")
parser.add_argument("--sleep", required=True, help="Path to sleep profile text file")
args = parser.parse_args()

filename = args.h5
targetID = args.sensor
...
for state, color in plot1_colors.items():
    ...
    for block_id in block_ids.unique():
        ...
        ax.axvspan(
            block_times.min(),
            block_times.max() + pd.Timedelta('1s'),
            color=color,
            alpha=0.25,
            zorder=0
        )
...
legend1 = (
    [Patch(facecolor=color, alpha=0.25, label=state.upper()) for state, color in plot1_colors.items()] +
    [Patch(facecolor='magenta', alpha=0.5, label='CoV')]
)

ax.legend(handles=legend1, loc='upper right')
ax.set_title("Continuous Movement Variability (CoV) Across Sleep and Wake")
ax.set_xlabel("Time")
...
legend2 = (
    [Patch(facecolor=color, alpha=0.25, label=state.upper()) for state, color in sleep_colors.items()] +
    [Patch(facecolor='magenta', alpha=0.5, label='CoV')]
)

ax2.legend(handles=legend2, loc='upper right')
ax2.set_title("REM Sleep Movement CoV Aligned to Pseudo-Timeline")
ax2.set_xlabel("Compressed Sleep Time")
ax2.set_ylabel("Coefficient of Variation")
ax2.grid(True)
ax2.set_xlim(sleep_cov_compressed.index.min(), sleep_cov_compressed.index.max())
